...
func load_fish_markers():
	...
	for n in fish_markers_to_load:
		var new_marker = fish_marker.instantiate()
		add_child(new_marker)
		var random_number = randi_range(0, fishable_tiles.keys().size() - 1)
		var random_cell = fishable_tiles.keys()[random_number]
		while tiles_with_markers.find(random_cell) != -1:
			random_cell = fishable_tiles.keys()[random_number]
